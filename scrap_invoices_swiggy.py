...
imapSession.select()
typ, data = imapSession.search(None, '(SUBJECT "Your Swiggy order no.")')
print(typ, data)
print('Search...')
counter = 0
...
for msgId in data[0].split():
    ...
    for part in mail.walk():
        ...
        if bool(fileName):
            ...
            filePath = os.path.join(detach_dir, 'attachments', fileName + '.pdf')
            # print(filePath)
            # imgkit.from_string(part.get_payload(), filePath)
            import re

            url = re.search("(?P<url>https?://[^\s]+invoice[^\"\s]+)", part.get_payload(decode=True).decode()).group(
                "url")
            # print(url)
            ...
            elif 'Atlanta' not in text and 'Vaishnavi' not in text:
                print (filePath)
                os.remove(filePath)
